using Becoming.Core.Common.Infrastructure.DataAccess.Repositories;
...
public sealed class TaskManagerRepository : BaseRepository<TaskManagerContext, TaskManagerSaveModel>, ITaskManagerRepository
{
    ...
    public async Task RequiredRelationUpdateAsync(TaskManagerSaveModel model, Guid targetId, IDbContextTransaction? transaction = default, CancellationToken token = default)
    {
    ...
    public async Task RequiredRelationUpdateAsync(TaskManagerSaveModel model, Guid targetId, Guid[] spares, IDbContextTransaction? transaction = default, CancellationToken token = default)
    {
        using var newTransaction = transaction ?? await _context.Database.BeginTransactionAsync(token);
    ...
    public Task<int> UseOriginalUpdateAsync(TaskManagerSaveModel changedDataModel, TaskManagerSaveModel? orinalModel = null, CancellationToken token = default)
    {
        throw new NotImplementedException();
    }
    #endregion
